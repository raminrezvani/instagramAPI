...
class CrawlInstagram:
    ...
    def process_media_with_ai(self, media_type='video'):
        ...
        try:
            ...
            print(f"Found {len(files)} {media_type} files...")
            start_time = datetime.now()
        
            # Process files in parallel
            futures = []
            for file_path in files:
                # Extract ID from filename (video_ID.mp4 or image_ID.jpg)
                file_id = file_path.stem.split('_', 1)[1]  # Gets the ID part after video_ or image_
                
                # Check if this media has already been analyzed
                existing_analysis = self.posts_collection.find_one(
                    {"id": file_id, "analysis_date": {"$exists": True}}
                )
                ...
                print(f"Queuing {file_path.name} for analysis")
                futures.append(
            ...
            print(f"Processing {len(futures)} new files...")
    ...
    def google_ai_perform_and_save(self, video_path, videoID):
        ...
        try:
            ...
            #Save to MongoDB
            self.posts_collection.update_one(
                {"id": videoID.replace('video_','').replace('.mp4','')},
                {
                    "$set": ai_result,
                },
                upsert=True
